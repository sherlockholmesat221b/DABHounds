...
def main():
    ...
    parser.add_argument(
        "--library-name",
        help="Override DAB library name (useful for file input)",
    )
    ...
    if file_input and args.mode is None:
        # Prefer ISRC-only matching for batch link files.
        match_mode = "strict"
    ...
    if file_input:
        ...
        for url in normalized_links:
            if not is_spotify_url(url):
                print(f"[DABHound] Skipping non-Spotify URL in file: {url}")
                continue
            try:
                if fetcher is None:
                ...
                print(
                    "[DABHound] Private/restricted content or client error. Retrying with OAuth..."
                )
